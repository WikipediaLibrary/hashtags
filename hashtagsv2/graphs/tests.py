...
class StatisticsTest(TestCase):
    def setUp(self):
        ...
        for i in range(1, 6):
            HashtagFactory(
                hashtag="test_hashtag1",
                domain="en.wikipedia.org",
                username="a",
                rc_id=i,
                timestamp=datetime(2016, 2, 1),
            )
    ...
    def test_top_project_statistics(self):
        ...
        self.assertEqual(
            dict["projects"],
            ["en.wikipedia.org", "fr.wikipedia.org", "es.wikipedia.org"],
        )
    ...
    def test_edits_over_days(self):
        ...
        dict = loads(page_content)
        self.assertEqual(
            dict["time_array"], ["2016-02-01", "2016-02-02", "2016-02-03", "2016-02-04"]
        )
    ...
    def test_edits_over_months(self):
        ...
        for i in range(1, 5):
            HashtagFactory(
                hashtag="test_hashtag2", rc_id=20 + i, timestamp=datetime(2016, i, 1)
            )
        # Some more edits in same month
        HashtagFactory(
            hashtag="test_hashtag2", rc_id=1234, timestamp=datetime(2016, 1, 3)
        )
        HashtagFactory(
            hashtag="test_hashtag2", rc_id=12345, timestamp=datetime(2016, 1, 3)
        )
        HashtagFactory(
            hashtag="test_hashtag2", rc_id=123, timestamp=datetime(2016, 3, 7)
        )

        factory = RequestFactory()

        data = {"query": "test_hashtag2"}
        request = factory.get("/api/time_stats", data)
        response = views.time_statistics_data(request)
        page_content = response.content.decode("utf-8")
        dict = loads(page_content)
        self.assertEqual(
            dict["time_array"], ["Jan-2016", "Feb-2016", "Mar-2016", "Apr-2016"]
        )
    ...
    def test_edits_over_years(self):
        ...
        for i in range(1, 5):
            HashtagFactory(
                hashtag="test_hashtag3",
                rc_id=20 + i,
                timestamp=datetime(2015 + i, 2, 1),
            )
        # More edits in same year
        HashtagFactory(
            hashtag="test_hashtag3", rc_id=1234, timestamp=datetime(2016, 1, 3)
        )
        HashtagFactory(
            hashtag="test_hashtag3", rc_id=12345, timestamp=datetime(2017, 5, 3)
        )
        HashtagFactory(
            hashtag="test_hashtag3", rc_id=123, timestamp=datetime(2017, 5, 3)
        )

        factory = RequestFactory()

        data = {"query": "test_hashtag3"}
        request = factory.get("/api/time_stats", data)
        response = views.time_statistics_data(request)
        page_content = response.content.decode("utf-8")
        dict = loads(page_content)
    ...
    def test_time_csv_daily_view(self):
        factory = RequestFactory()
        data = {"query": "test_hashtag1", "view_type": "dailyTimeChart"}
    ...
    def test_time_csv_monthly_view(self):
        for i in range(1, 5):
            HashtagFactory(
                hashtag="test_hashtag4",
                rc_id=120 + i,
                timestamp=datetime(2016, i, 1 + i),
            )

        factory = RequestFactory()
        data = {"query": "test_hashtag4", "view_type": "monthlyTimeChart"}
        request = factory.get("/time_csv", data)
        response = views.time_csv(request)
        page_content = response.content
    ...
    def test_time_csv_yearly_view(self):
        for i in range(1, 5):
            HashtagFactory(
                hashtag="test_hashtag5",
                rc_id=120 + i,
                timestamp=datetime(2011 + i, i, i + 3),
            )

        factory = RequestFactory()
        data = {"query": "test_hashtag5", "view_type": "yearlyTimeChart"}
        request = factory.get("/time_csv", data)
        response = views.time_csv(request)
        page_content = response.content
